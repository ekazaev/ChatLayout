...
final class AppKitTestDataProvider: RandomDataProvider {
    weak var delegate: RandomDataProviderDelegate?
    ...
    private var startingTimestamp: TimeInterval = 1740752194
    ...
    private func handleTimer() {
        ...
        let message = createBunchOfMessages(number: 1).first!
        delegate?.received(messages: [message])
    ...
    private func restartMessageTimer() {
        messageTimer?.invalidate()
        messageTimer = nil
        messageTimer = Timer.scheduledTimer(timeInterval: TimeInterval(2), target: self, selector: #selector(handleTimer), userInfo: nil, repeats: true)
    }

    private func restartTypingTimer() {
        typingTimer?.invalidate()
        typingTimer = nil
        typingTimer = Timer.scheduledTimer(timeInterval: TimeInterval(3), target: self, selector: #selector(handleTypingTimer), userInfo: nil, repeats: true)
    }
//
//    private func createRandomMessage(date: Date = Date()) -> RawMessage {
//        let sender = allUsersIds[Int.random(in: 0 ..< allUsersIds.count)] // allUsersIds.first!//
...
//    }
    ...
    private func createBunchOfMessages(number: Int = 50) -> [RawMessage] {
        let messages = (0 ..< number).map { number -> RawMessage in
            startingTimestamp += TimeInterval(100)
//            return self.createRandomMessage(date: Date(timeIntervalSince1970: startingTimestamp))
            let sender = allUsersIds[number % allUsersIds.count]
            lastMessageIndex += 1
            let date = Date(timeIntervalSince1970: startingTimestamp)
            switch number {
//            case 0 ..< 5:
//                return RawMessage(
//                    id: UUID(),
//                    date: date,
//                    data: .text(TextGenerator.appkitGetString(of: 5) +
//                        " \(websiteUrls[number % websiteUrls.count]). " +
//                        TextGenerator.appkitGetString(of: 5)),
//                    userId: sender
//                )
//            case 10 ..< 15:
//                return RawMessage(id: UUID(), date: date, data: .url(websiteUrls[number % websiteUrls.count]), userId: sender)
//            case 25 ..< 30:
//                return RawMessage(id: UUID(), date: date, data: .image(.imageURL(imageUrls[number % imageUrls.count])), userId: sender)
//            case 40 ..< 45:
//                return RawMessage(id: UUID(), date: date, data: .image(.image(images[number % images.count])), userId: sender)
            default:
                return RawMessage(id: UUID(), date: date, data: .text(TextGenerator.appkitGetString(of: 20)), userId: sender)
            }
        }
        return messages
    }
